...
# Verify WeasyPrint works - DO NOT touch symlinks, just verify paths
RUN python3 --version && weasyprint --version && \
    echo "Python3 at: $(which python3) -> $(readlink -f $(which python3))" && \
    echo "WeasyPrint at: $(which weasyprint) -> $(readlink -f $(which weasyprint))"

# Set PATH globally so Apache child processes can find python3 and weasyprint
